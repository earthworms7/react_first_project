...
class Td extends Component {
    ...
    setCheckList = (type, count, level_count, e) => {
        this.props.setCheckList(type, count, level_count, e.target.checked);
    }
    ...
    level_check_td = () => {
        const levelClearCheckList = this.props.levelClearCheckList;
        const moreCheckList = this.props.moreCheckList;
        const item = this.props.item;
        const count = this.props.count;
        const rewardList = item.reward;
        const max_level = item.level[item.level.length-1].id;
        ...
        const check_td = rewardList.map(reward => {
            ...
            const lCount = level_count;
            let keyNum = 2 + reward.id;

            if( max_level !== 3 && reward.id === max_level){
                return (
                    <td key={`td_check_${count}_${keyNum}`} className="result_check_td" colSpan={2}>
                        <input type="checkbox" checked={levelClearCheckList[count-1][lCount]} onChange={(e) => this.setCheckList("l", count, lCount, e)}/> 관문진행<br/>
                        <input type="checkbox" checked={moreCheckList[count-1][lCount]} onChange={(e) => this.setCheckList("m", count, lCount, e)}/> 더보기
                    </td>
                )
            } else {
                return (
    ...
    tr_check_result = ({item, count}) => {
        ...
        return (
            <tr key={`tr_check_${count}`}>
                <td key={`td_check_${count}_1`} ><input type="checkbox" id={`contentClearCheck_${count}`} checked={contentClearCheckList[count-1]} onChange={(e) => this.setCheckList("c", count, 0, e)}/></td>
                {this.level_check_td()}
            </tr>
        );
    }
